...
export function RecentlyViewed() {
  ...
  return (
    <div className="flex items-center gap-2 overflow-x-auto scrollbar-thin py-2">
      ...
      {items.map((it) => (
        <span key={it.href} className="group inline-flex items-center gap-1.5 rounded-full bg-white/5 ring-1 ring-white/15 px-2.5 py-1 text-xs whitespace-nowrap shrink-0 hover:bg-white/10">
          <Link href={it.href} className="text-white/85 hover:text-white">{it.label}</Link>
          <button onClick={() => remove(it.href)} aria-label="Remove" className="text-white/30 hover:text-white opacity-0 group-hover:opacity-100 transition">
            <X className="h-3 w-3" />
          </button>
        </span>
      ))}
