...
/*  BUTTONS                                                           */
/* ================================================================== */
function MagneticButton({ children, primary = true, compact = false, onClick }: {
  children: ReactNode; primary?: boolean; compact?: boolean; onClick?: () => void;
}) {
  const ref = useMagnetic<HTMLButtonElement>(0.28);
  const sty: CSSProperties = compact
    ...
    <nav className="sticky top-0 z-30 w-full backdrop-blur-xl"
      ...
      <div className="max-w-6xl mx-auto px-5 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-7 text-sm" style={{ fontFamily: SANS }}>
          {["Features", "Reviews", "Pricing", "FAQ"].map((l) => (
            <a key={l} className="relative group cursor-pointer" style={{ color: P.muted }}>
              <span className="group-hover:text-white transition-colors duration-200">{l}</span>
...
function DashboardMockup() {
  ...
  return (
    <section className="px-3 sm:px-5 pb-12">
      <Reveal delay={80}>
          ...
          <Reveal key={i} delay={i * 100}>
            <SpotlightCard popular={p.popular} className="p-5 h-full">
              ...
              <div className="relative">
                ...
                <button className="w-full py-2 rounded-lg text-sm transition-all hover:scale-[1.02]"
                  style={{
