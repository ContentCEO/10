...
import {
  Calendar, ChevronRight, Clock, Eye, MapPin, Sparkles, Wallet, Zap,
} from "lucide-react";
import {
  BUDGET_LABELS,
  TIMELINE_LABELS,
...
function scoreColor(score: number) {
  if (score >= 85) return "#fb7185"; // rose-400 — HOT
  if (score >= 70) return "#10b981"; // emerald — strong
  if (score >= 50) return "#f59e0b"; // amber  — decent
  return "#94a3b8";
}
...
export function MarketplaceCard({
  ...
  const fresh = (Date.now() - new Date(lead.created_at).getTime()) < 60 * 60 * 1000; // <1h

  return (
    <li className="group relative overflow-hidden rounded-2xl flex flex-col gap-3 p-5 transition-all duration-200 hover:-translate-y-0.5"
      style={{
        background: hot
      ...
      {/* Top accent stripe for hot leads */}
      {hot && (
      ...
      {/* Header — trade + location + AI score */}
      ...
      <div className="flex items-center justify-between border-t border-white/5 pt-3 -mx-1 px-1">
        <div>
          ...
          {!affordable && (
            <div className="text-[11px] text-rose-300 mt-1 inline-flex items-center gap-1">
              <Wallet className="h-3 w-3" /> Need {money(lead.price_cents - balanceCents)} more
            </div>
          )}
        </div>
