...
export default async function AutoOutreachDashboard() {
  ...
  const [totals, recentLogs, recentProspects, conversions] = await Promise.all([
    admin.from("cf_launchpad_prospects").select("scan_status,generate_status,outreach_status"),
    admin.from("cf_launchpad_outreach_log").select("id,channel,status,created_at,prospect_id").order("created_at", { ascending: false }).limit(10),
    admin.from("cf_launchpad_prospects").select("id,business_name,city,scan_status,generate_status,outreach_status,created_at").order("created_at", { ascending: false }).limit(10),
    admin.from("cf_launchpad_subscriptions").select("amount_cents,plan,status").eq("status", "active"),
  ]);
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          ...
          <Link
            href="/owner/launchpad/prospects"
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Panel title="Latest prospects" linkHref="/owner/launchpad/prospects" linkLabel="View all →">
          {recentProspects.data?.length ? (
            <ul className="divide-y divide-white/10">
              {recentProspects.data.map((p) => (
                <li key={p.id} className="py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <Link href={`/owner/launchpad/prospects/${p.id}`} className="font-semibold hover:underline truncate block">
                      {p.business_name}
                    </Link>
        ...
        <Panel title="Latest outreach" linkHref="/owner/launchpad/prospects" linkLabel="View all →">
