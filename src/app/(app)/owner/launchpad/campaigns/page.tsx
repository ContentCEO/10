...
export default async function CampaignsPage() {
  ...
  const { data: campaigns } = await admin.from("cf_launchpad_campaigns")
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      {!campaigns?.length && (
        <div className="rounded-2xl border border-dashed border-white/15 p-8 text-center text-sm text-white/60">
          ...
          (Add UI for create/edit in a follow-up — for now, insert rows directly into <code className="bg-white/10 px-1.5 py-0.5 rounded text-xs">cf_launchpad_campaigns</code>.)
        </div>
      )}
