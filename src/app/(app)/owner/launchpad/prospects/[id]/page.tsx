...
import { buildPreviewUrl } from "@/lib/launchpad/pipeline";
import {
...
export default async function ProspectDetail({ params }: Params) {
  ...
  const { data: prospect } = await admin.from("cf_launchpad_prospects").select("*").eq("id", params.id).maybeSingle();
  if (!prospect) return notFound();

  const [{ data: scan }, { data: site }, { data: ads }, { data: logs }] = await Promise.all([
    admin.from("cf_launchpad_scans").select("*").eq("prospect_id", prospect.id)
      .order("created_at", { ascending: false }).limit(1).maybeSingle(),
    admin.from("cf_launchpad_sites").select("*").eq("prospect_id", prospect.id).maybeSingle(),
    admin.from("cf_launchpad_ad_strategies").select("*").eq("prospect_id", prospect.id),
    admin.from("cf_launchpad_outreach_log").select("*").eq("prospect_id", prospect.id)
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <Link href="/owner/launchpad/prospects" className="text-xs text-white/60 hover:text-white">
            ← All prospects
          </Link>
