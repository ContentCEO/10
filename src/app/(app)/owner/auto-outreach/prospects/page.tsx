...
export default async function ProspectsList({ searchParams }: { searchParams: Search }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex gap-2 items-center flex-wrap" action="/owner/auto-outreach/prospects">
        <input
          name="search"
        ...
        <Link href="/owner/auto-outreach/discover" className="rounded-lg bg-brand-gradient px-3 py-2 text-sm font-semibold">
          + Discover more
        </Link>
      </form>
            ...
            {(prospects ?? []).map((p) => (
              <tr key={p.id} className="hover:bg-white/5">
                <td className="px-4 py-3">
                  <Link href={`/owner/auto-outreach/prospects/${p.id}`} className="font-semibold hover:underline">
                    {p.business_name}
                  </Link>
                </td>
