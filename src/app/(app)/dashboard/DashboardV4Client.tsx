...
/*  PALETTE & FONTS                                                   */
/* ================================================================== */
// Dark indigo palette to match the landing page.
const P = {
  bg:        "#06060A",
  surface:   "#11111A",
  surface2:  "#1B1B26",
...
interface Props {
  ...
  /** When true, skip the v4 TopBar/Sidebar/CursorGlow chrome (the host layout
   * provides its own). Used when promoted to /dashboard inside the (app) shell. */
  embedded?: boolean;
}
...
export default function DashboardV4Client(props: Props) {
  ...
  return (
    <>
      ...
      <div style={{ background: props.embedded ? "transparent" : P.bg, color: P.text,
                    minHeight: props.embedded ? "auto" : "100vh",
        ...
        <div className={props.embedded
          ? "flex gap-5"
          : "max-w-[1320px] mx-auto px-5 flex gap-5"}>
          {!props.embedded && <Sidebar />}

          <main className={props.embedded
            ? "flex-1 min-w-0 space-y-5"
            : "flex-1 min-w-0 py-6 space-y-5"}>
