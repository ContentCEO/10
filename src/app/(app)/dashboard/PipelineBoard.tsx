          ...
          >
            <div className={`relative overflow-hidden rounded-lg bg-gradient-to-br ${col.tone} px-3 py-2 mb-3`}>
              ...
              <div className="mt-1 flex items-baseline gap-2">
                ...
                {total > 0 && <span className="text-xs text-white/60 tabular-nums">${total.toLocaleString()}</span>}
              </div>
            </div>
            <div className="space-y-1.5">
              {colLeads.length === 0 && (
                <div className="text-xs text-white/30 text-center py-4 border border-dashed border-white/5 rounded-lg">Drop a lead here</div>
              )}
              {colLeads.map((l) => (
                <div
                  key={l.id}
                  ...
                  className={
                    "group flex items-start gap-2 rounded-lg bg-white/[0.04] ring-1 ring-white/10 p-2.5 cursor-grab active:cursor-grabbing hover:bg-white/[0.08] transition" +
                    (pending && dragId === l.id ? " opacity-50" : "")
                  }
                >
                  <GripVertical className="h-3 w-3 text-white/25 mt-0.5 shrink-0 group-hover:text-white/50 transition" />
                  <Link href={`/leads/${l.id}`} className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-white truncate">{l.name}</div>
                    <div className="text-[11px] text-white/55 truncate">{l.service_type ?? "—"}</div>
