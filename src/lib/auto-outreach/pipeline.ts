...
export async function runScan(prospectId: string): Promise<ScanResult> {
  ...
  const { data: prospect, error } = await supabase
    .from("ao_prospects").select("*").eq("id", prospectId).single();
  if (error || !prospect) throw new Error(`Prospect not found: ${prospectId}`);

  await supabase.from("ao_prospects")
  ...
  try {
    ...
    await supabase.from("ao_scans").insert({
...
export async function runGenerate(prospectId: string): Promise<{ slug: string; site: SiteContent }> {
  ...
  let { data: scan } = await supabase
    .from("ao_scans").select("*").eq("prospect_id", prospectId)
  ...
  try {
    ...
    await supabase.from("ao_sites").upsert(
      {
        prospect_id: prospect.id,
        slug,
    ...
    await supabase.from("ao_ad_strategies").delete().eq("prospect_id", prospect.id);
    for (const s of adStrategies) {
      await supabase.from("ao_ad_strategies").insert({
        prospect_id: prospect.id,
        platform: s.platform,
...
async function ensureUniqueSlug(base: string): Promise<string> {
  ...
  let n = 1;
  while (true) {
    const { data } = await supabase.from("ao_sites").select("id").eq("slug", slug).maybeSingle();
    if (!data) return slug;
    n += 1;
    slug = `${base}-${n}`;
...
export async function runOutreach(prospectId: string, channel: Channel, campaignId?: string | null) {
  ...
  const { data: site } = await supabase
    .from("ao_sites").select("slug").eq("prospect_id", prospectId).maybeSingle();
  if (!site) throw new Error("No generated site yet — run generate first");

  const { data: scan } = await supabase
    .from("ao_scans").select("summary, weaknesses").eq("prospect_id", prospectId)
    .order("created_at", { ascending: false }).limit(1).maybeSingle();

  let campaignTemplate: { subject?: string | null; body: string } | null = null;
  if (campaignId) {
    const { data: c } = await supabase
      .from("ao_campaigns").select("subject_tpl, body_tpl").eq("id", campaignId).single();
...
export function buildPreviewUrl(slug: string): string {
  ...
  if (sitesDomain) return `https://${slug}.${sitesDomain}`;
