...
// shared messaging.ts helpers; logs every send to ao_outreach_log.
...
async function aiPersonalize(input: PersonalizeInput): Promise<PersonalizedMessage> {
  ...
  const system = channel === "email"
    ? "You write short, casual, hyper-personal cold emails to local-business owners. " +
      "Tone: like one human helping another. No 'I hope this finds you well'. " +
      "No bullet lists. 60-90 words max. Output JSON only."
    : "You write short, friendly text messages to local-business owners. " +
      "Tone: helpful neighbor, not pushy salesperson. 280 chars max. Output JSON only.";
...
function fallback(input: PersonalizeInput): PersonalizedMessage {
  ...
  const body = channel === "email"
    ...
    : `Hey ${p.business_name} — built you a quick personalized site preview: ${previewUrl}. No signup, free to check. Reply STOP to opt out.`;
  return {
...
export async function sendOutreach(opts: SendOutreachOptions) {
  ...
  const send = channel === "email"
    ? await sendEmail(to, msg.subject ?? `Quick mockup for ${p.business_name}`, msg.body, "ContractorFlow")
